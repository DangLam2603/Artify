...
const UpdatePost = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const init = {
        ...result.data.payload,
        artList: await Promise.all(
          result.data.payload.artList.map(async (art) => {
            ...
            if (art.imagePath) {
              ...
              return {
                ...art,
                imageFile: file,
                imageUrl: art.imagePath,
              };
            } else {
              // Nếu không có imagePath, không cần tải hình ảnh
              return {
                ...art,
                imageFile: null,
                imageUrl: "",
  ...
  console.log(post !== null ? post : "aaa");
  ...
  const handleSubmit = async (values) => {
    ...
    const response = await axios.put(
      "http://localhost:8080/api/auth/updatePost",
      formData,
      {
        headers: { Authorization: `Bearer ${token}` },
      }
                  ...
                  <Form className="w-full px-32">
                    {current == 0 ? (
                      ...
                      <FieldArray name="artList">
                        {({ push, remove }) => (
                          <div>
                            <h3 className="text-3xl text-[#2f6a81] text-center font-semibold mb-5">
                              Art List
                            </h3>
